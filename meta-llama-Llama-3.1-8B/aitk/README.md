...
This workflow is configured using the `llama3_1_qnn_config.json` file. It contains all of the quantization and compilation steps. It requires two separate Python environments described below.
...
Replace `/path/to/qnn/env/bin` in `llama3_1_qnn_config.json` with the path to the directory containing your QNN environment's Python executable. This path can be found by running the following command in the environment:
...
```bash
olive run --config llama3_1_qnn_config.json
```
...
✅ Optimized model saved in: `./model/llama3_1`
...
### **VitisAI QDQ (AMD NPU)**
```bash
olive run --config llama3_1_vitis_ai_config.json
```

### **OpenVINO (Intel NPU)**
```bash
olive run --config llama3_1_ov_config.json
```

### **DML (GPU - AutoAWQ)**
```bash
olive run --config llama3_1_dml_config.json
```
