...
def main(raw_args=None):
    ...
    parser.add_argument(
        "--provider", default="cuda", type=str, choices=["cuda", "cpu", "qnn"], help="Execution provider to use"
    )
    parser.add_argument(
    ...
    if args.provider == "cuda" and version.parse(OrtVersion) < version.parse("1.17.0"):
    ...
    optimized_dir_name = f"optimized-{args.provider}"
    if args.format:
