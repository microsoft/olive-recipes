# LiquidAI-LFM2.5-Audio-1.5B — CPU optimization
...
#### `_cpu_int4.json` — Q4_0 equivalent
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_cpu_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_cpu_int8.json
```

Both set `exclude_embeds` (the token table lives in `embedding.onnx`, so the encoder features can be
spliced in), `include_hidden_states` (the depthformer reads them) and `accuracy_level: 4`:
activations are quantized to INT8, as llama.cpp does for Q4_0 on CPU. Without it decode is 6 to 60
times slower (INT4: 103 against 8 tok/s on a Xeon Platinum 8358, 212 against 34 on an Apple M3
Ultra; INT8: 78 against 1.3 and 120 against 8).
...
#### `_cpu_audio_int4.json` — Q4_0 equivalent
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_cpu_audio_int4.json
```

#### `_cpu_audio_int8.json` — Q8_0 equivalent
INT8 throughout, with `accuracy_level: 4` (INT8 activations): with FP32 activations the 8-bit CPU
kernels made the depthformer 3 to 4 times slower (121 against 43 ms per frame on a Xeon Platinum
8358, 75 against 20 ms on an Apple M3 Ultra).

```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_cpu_audio_int8.json
```
...
ASR is word error rate on 100 random LibriSpeech test-clean clips (11.6 minutes). TTS is 20
sentences spoken with the model card's sampling, transcribed by Whisper large-v3-turbo and scored
against the text. Both use Whisper's English normalizer. The GGUF rows are LiquidAI's release run
through its llama.cpp runner.

| recipe | size | ASR WER, Xeon / M3 Ultra | TTS WER, Xeon / M3 Ultra |
| --- | --- | --- | --- |
| int4 | 1.31 GB | 2.10% / 2.15% | 0.00% / 0.00% |
| int8 | 1.97 GB | 2.10% / 2.10% | 0.00% / 0.00% |
| FP32 (reference) | 6.52 GB | 2.10% | 0.88% |
...
Fidelity against the FP32 ONNX pipeline, teacher-forced (so every graph sees the same inputs): the
decoder over 912 positions of 8 chat answers, the encoder over the 2282 output frames of 30 clips,
the depthformer over the 3480 codes of 435 spoken frames, the detokenizer on the same codes.
...
| int8 | 0.993 / 0.0002 | 0.9998 (0.9997) | 0.989 / 0.0003 | 0.19 dB |

The INT8 embedding table costs nothing measurable: with the FP32 decoder it keeps every top-1 token,
at a KL of 1e-6. The numbers above are from the Xeon; the M3 Ultra's are within 0.003.

Speed through ONNX Runtime GenAI (decode: greedy text; real-time factor: generation time over audio
duration, lower is faster; depthformer: one 8-code frame, 80 ms of speech):

| recipe | machine | decode | ASR RTF | TTS RTF | depthformer |
| --- | --- | --- | --- | --- | --- |
| int4 | Xeon Platinum 8358, 30 cores | 104 tok/s | 0.069 | 0.67 | 41 ms |
| int8 | Xeon Platinum 8358, 30 cores | 75 tok/s | 0.081 | 0.75 | 43 ms |
| int4 | Apple M3 Ultra | 196 tok/s | 0.034 | 0.39 | 22 ms |
| int8 | Apple M3 Ultra | 111 tok/s | 0.046 | 0.39 | 20 ms |
