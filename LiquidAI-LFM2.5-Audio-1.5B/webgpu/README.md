# LiquidAI-LFM2.5-Audio-1.5B — WebGPU optimization
...
#### `_webgpu_int4.json` — Q4_0 equivalent
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_webgpu_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_webgpu_int8.json
```
...
activations stay in floating point, as in llama.cpp's GPU kernels. INT8 activations did not make
decode any faster on an Apple M3 Ultra, and tripled the INT8 decoder's KL against FP32 (0.0015
against 0.0005).
...
#### `_webgpu_audio_int4.json` — Q4_0 equivalent
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_webgpu_audio_int4.json
```

#### `_webgpu_audio_int8.json` — Q8_0 equivalent
INT8 throughout. The encoder and the depthformer run on CPU in a WebGPU package (see below), so they
use `accuracy_level: 4` (INT8 activations) as on CPU: with FP32 activations the 8-bit CPU kernels
made the depthformer 3 to 4 times slower (121 against 43 ms per frame on a Xeon Platinum 8358, 75
against 20 ms on an Apple M3 Ultra).

```
olive run --config LiquidAI-LFM2.5-Audio-1.5B_webgpu_audio_int8.json
```
...
`inference.py` registers the WebGPU plug-in execution provider from `onnxruntime-ep-webgpu`; the
`onnxruntime` wheels do not ship WebGPU themselves.

## Running on WebGPU

The decoder and the embedding model run on WebGPU. `finalize.py` keeps the speech encoder, the
depthformer and the audio embedding on CPU:

- The depthformer grows its KV cache by one position for each of the 8 codes of a frame, which ONNX
  Runtime's WebGPU `GroupQueryAttention` does not always handle
  ([microsoft/onnxruntime#32716](https://github.com/microsoft/onnxruntime/issues/32716)). Moved to
  WebGPU, the depthformer answered a spoken question with the same text but only 18% of the audio
  codes it gives on CPU under Dawn over Vulkan (llvmpipe, Linux); on an Apple M3 Ultra (Metal) the
  two placements agreed. On CPU it is right on both.
- The encoder runs once per clip and the audio embedding is a table lookup; neither is worth a GPU.

Speech *input* in this layout needs
[microsoft/onnxruntime-genai#2616](https://github.com/microsoft/onnxruntime-genai/pull/2616), which
lets the encoder run on its own device; without it the runtime refuses a prompt with audio while the
encoder is on CPU. TTS and typed prompts work either way.

## Measured quality and speed

Measured on an Apple M3 Ultra (Dawn over Metal). ASR is word error rate on 100 random LibriSpeech
test-clean clips (11.6 minutes). TTS is 20 sentences spoken with the model card's sampling,
transcribed by Whisper large-v3-turbo and scored against the text. Both use Whisper's English
normalizer.

| recipe | size | ASR WER | TTS WER | decode | ASR RTF | TTS RTF |
| --- | --- | --- | --- | --- | --- | --- |
| int4 | 1.22 GB | 2.00% | 0.00% | 229 tok/s | 0.026 | 0.38 |
| int8 | 1.88 GB | 2.10% | 0.44% | 185 tok/s | 0.028 | 0.35 |
...
Fidelity of the decoder on WebGPU against the FP32 ONNX decoder, teacher-forced over 912 positions
of 8 chat answers: top-1 0.941 and KL 0.023 for INT4, 0.992 and 0.0005 for INT8. The embedding model
on WebGPU keeps every top-1 token at a KL of 1e-6. The graphs that run on CPU match the CPU recipes'
numbers (see `../cpu/README.md`).

On Linux both builds were also run through Dawn over Vulkan on a software device (no hardware Vulkan
driver was available): they transcribed and spoke correctly, which checks the graphs but says
nothing about speed.
...
- `onnxruntime-genai` with LFM2-Audio support,
  ...
  wheel from `main`.
- `onnxruntime-ep-webgpu` 0.4.0 or newer, the WebGPU plug-in for the `onnxruntime` 1.30 wheels.
- For speech *input* on WebGPU,
  [microsoft/onnxruntime-genai#2616](https://github.com/microsoft/onnxruntime-genai/pull/2616).
  Without it the runtime refuses a prompt with audio while the encoder runs on CPU; TTS and typed
  prompts work without it.
