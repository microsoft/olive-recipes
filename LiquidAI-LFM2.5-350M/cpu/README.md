...
| --- | --- | --- | --- |
| `_cpu_int4.json` | 0.54 GiB | 0.492 | 67.5% |
| `_cpu_int8.json` | 0.47 GiB | 0.0090 | 95.4% |
| llama.cpp Q4_K_M | 0.21 GiB | 0.465-0.481 | 67.4-68.0% |
| llama.cpp Q8_0 | 0.35 GiB | 0.0037-0.0084 | 95.3-96.7% |

INT8 matches Q8_0 (0.0090 against 0.0084).

INT4 matches Q4_K_M's quality (0.492 against 0.481 on llama.cpp's CPU backend), at 2.6x its size.
The size comes from two places: the embedding table stays in FP32 next to the INT8 LM head, where
Q4_K_M keeps one 6-bit copy, and MatMulNBits stores an FP32 scale for every block of 32 weights,
where Q4_K packs 6-bit scales into 256-weight super-blocks. On this model that makes INT4 larger
than INT8 (0.47 GiB).
