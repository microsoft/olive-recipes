# Package Phi-4-mini-reasoning into an ONNX Runtime ortpackage

This recipe walks through the end-to-end flow for taking a Hugging Face
model (`microsoft/Phi-4-mini-reasoning`), exporting it to ONNX with
Olive's model builder for both CPU and GPU, and bundling the two
exports into a single `.ortpackage` that the ONNX Runtime GenAI runtime
can load and switch between at inference time.
...
```bash
olive capture-onnx-graph -m microsoft/Phi-4-mini-reasoning --use_model_builder -o gpu
```

This downloads the HF model, runs the model builder, and writes an ONNX
source under `gpu/`.
...
```bash
olive capture-onnx-graph -m microsoft/Phi-4-mini-reasoning --use_model_builder -o cpu --precision fp32
```
