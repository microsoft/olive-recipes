# LiquidAI-LFM2.5-VL-450M — WebGPU optimization
...
```
olive run --config LiquidAI-LFM2.5-VL-450M_webgpu_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-VL-450M_webgpu_fp16_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-VL-450M_webgpu_vision_int8.json
```
...
| --- | --- | --- | --- |
| `_webgpu_int4.json` | 0.33 GiB | 0.066 | 86.0% |
| `_webgpu_fp16_int4.json` | 0.36 GiB | 0.147 | 79.5% |
| llama.cpp Q4_K_M | 0.21 GiB | 0.062-0.067 | 86.2-86.7% |
| llama.cpp Q8_0 | 0.35 GiB | 0.0006-0.0013 | 97.9-98.7% |

INT4 matches Q4_K_M's quality (0.066 against 0.067 on llama.cpp's CPU backend), at 1.6x its size.
...
`_webgpu_fp16_int4.json` is the least accurate recipe here despite being the larger one (KLD 0.147
against 0.066): plain RTN costs more than the FP16 LM head saves.
