...
# -------------------------------------------------------------------------
"""
NPU (VitisAI) pipeline save and load: save_vai_pipeline, get_vai_pipeline.
Unet/vae_decoder use DD session options (dd_cache, VitisAI EP); other submodels use OnnxRuntimeModel.from_pretrained.
"""
...
def get_vai_pipeline(model_dir, common_args):
    ...
    print(
        "Loading NPU pipeline (unet/vae_decoder from dd/replaced.onnx with VitisAI EP, "
        "text_encoder/vae_encoder with CPU EP)..."
    )
    ...
    if (model_dir / "safety_checker").exists():
        try:
            safety_checker = OnnxRuntimeModel.from_pretrained(
                str(model_dir / "safety_checker"), providers=["CPUExecutionProvider"], provider_options=[{}]
            )
        except Exception:
            pass
