...
class PreprocessSplit:
    def __init__(
        self, tokenizer, block_size, column_name="text", add_special_tokens=True
    ):
        self._tokenizer = tokenizer
        self._block_size = block_size
    ...
    def _group_texts(self, examples):
        ...
        result = {
            k: [
                t[i : i + self._block_size]
                for i in range(0, total_length, self._block_size)
            ]
...
def get_wiki_dataset(block_size, tokenizer, cache_dir, path=None):
    if path is not None:
        ...
        dataset["test"] = load_dataset(
            path="wikitext", name="wikitext-2-raw-v1", cache_dir=cache_dir, split="test"
        )

    dataset["train"] = PreprocessSplit(tokenizer, block_size).preprocess(
        dataset["train"]
    )
    dataset["test"] = PreprocessGptqSplit(tokenizer, block_size).preprocess(
        dataset["test"]
    )

    train_dataloader = DataLoader(
        dataset["train"], shuffle=False, batch_size=1, collate_fn=default_data_collator
    )
    test_dataloader = DataLoader(
        dataset["test"], shuffle=False, batch_size=1, collate_fn=default_data_collator
    )
