...
def apply_input_output_exception(quantizer, exception):
    assert isinstance(exception, dict), (
        f"Exception is not a dictionary type: {exception}"
    )
    ...
    if quantizer is None:
        quantizer = LazyQuantizer(
            exception.get("bitwidth", 16),
            "nearest",
    ...
    if "bitwidth" in exception.keys():
        if isinstance(quantizer, FloatQuantizeDequantize):
            assert exception["bitwidth"] in [8, 16], (
                "Bitwidth for FloatQuantizeDequantize should only be 8 or 16"
            )
    ...
    if "asymmetric" in exception.keys():
        ...
        assert (quantizer._symmetric is True and quantizer._signed is True) or (
            quantizer._symmetric is False and quantizer._signed is False
        ), "symmetric and signed must be aligned (True or False)"
    ...
    if "encoding_overrides" in exception.keys():
        ...
        if not "is_symmetric" in encodings.keys():
            assert hasattr(quantizer, "_symmetric"), (
                f'Quantizer {quantizer} doesn\'t have attribute "_symmetric"'
            )
        ...
        if not "bitwidth" in encodings.keys():
            assert hasattr(quantizer, "bitwidth"), (
                f'Quantizer {quantizer} doesn\'t have attribute "bitwidth"'
            )
...
def apply_param_exception(quantizer, exception):
    ...
    assert (quantizer.symmetric is True and quantizer.signed is True) or (
        quantizer.symmetric is False and quantizer.signed is False
    ), "symmetric and signed must be aligned (True or False)"
        ...
        with open(mixed_precision_config_file) as f:
            ...
            for etype in exception_types:
                for item in exception_config[f"{etype}_list"]:
                    expections_str = {
                        k: v for k, v in item["exceptions"].items() if v is not None
                    }
        ...
        for etype in ("module", "name"):
            ...
            for name, module in quant_sim.model.named_modules():
                if isinstance(module, BaseQuantizationMixin):
                    exception = None
                    if etype == "module":
                        exception = get_module_exception(
                            module, self.exceptions_dict[etype]
                        )
                    elif etype == "name":
                    ...
                    if exception is not None:
                        if exception["param_exceptions"] is not None:
                            self.apply_param_exception_to_module(
                                exception["param_exceptions"], module
                            )

                        if exception["input_exceptions"] is not None:
                            self.apply_input_exception_to_module(
                                exception["input_exceptions"], module
                            )

                        if exception["output_exceptions"] is not None:
                            self.apply_output_exception_to_module(
                                exception["output_exceptions"], module
                            )

    def apply_param_exception_to_module(self, param_exceptions, module):
        is_enable = (
            (int(param_exceptions["bitwidth"]) < 32)
            if "bitwidth" in param_exceptions
            else True
        )
        if not is_enable:
            param_exceptions["enabled"] = is_enable

        module.param_quantizers["weight"] = apply_param_exception(
            module.param_quantizers["weight"], param_exceptions
        )

    def apply_input_exception_to_module(self, input_exceptions, module):
        for index in range(len(input_exceptions)):
            module.input_quantizers[index] = apply_input_output_exception(
                module.input_quantizers[index], input_exceptions[index]
            )

    def apply_output_exception_to_module(self, output_exceptions, module):
        for index in range(len(output_exceptions)):
            module.output_quantizers[index] = apply_input_output_exception(
                module.output_quantizers[index], output_exceptions[index]
            )
