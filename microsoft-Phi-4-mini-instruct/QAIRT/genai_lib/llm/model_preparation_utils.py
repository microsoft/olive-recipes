...
def llm_build_preparer_converter_args(
    num_hidden_layers,
    model_input_names,
    separate_position_ids=True,
    use_qairt_mpp=False,
    ...
    if not use_qairt_mpp:
                ...
                if input_name == "position_ids" and separate_position_ids:
                    ...
                    for i in range(num_hidden_layers):
                        if i in attention_indices:
                            converter_args += [
                                input_param,
                                f"past_key_values[{i}][0]",
                                converter_args_value,
                            ]
                            converter_args += [
                                input_param,
                                f"past_key_values[{i}][1]",
                                converter_args_value,
                            ]
                        else:
                            converter_args += [
        ...
        for input_name in model_input_names:
            if (
                input_name in {"position_ids", "swa_position_ids"}
                and separate_position_ids
            ):
                for i in range(2):
                    input_tensor_config = InputTensorConfig(
                        name=f"{input_name}[{i}]",
                        source_model_input_layout="NONTRIVIAL",
                        source_model_input_datatype="float32",
                    )
                ...
                input_tensor_config = InputTensorConfig(
                    name=input_name,
                    source_model_input_layout="NONTRIVIAL",
                    source_model_input_datatype="int64",
                )
                input_tensors.append(input_tensor_config)
