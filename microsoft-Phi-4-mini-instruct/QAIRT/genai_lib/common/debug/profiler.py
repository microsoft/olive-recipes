...
class ProfileMarker:
    ...
    def __str__(self):
        ...
        return (
            f"Event {self.event} : time={timedelta(seconds=self.time)}, "
...
def ram_watermark_function(
    ram_allocated: Value, pid: int, polling_interval_in_ms: float
):
    """
...
class EventProfiler:
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            ...
            logger.info(
                "Created Latency/Memory profiler: empty_cache=%s",
                cls._instance._empty_cache,
            )  # pylint: disable=protected-access
    ...
    def empty_cache(self, enable: bool = False):
        ...
        if self._empty_cache != enable:
            if self._empty_cache:
                logger.warning(
                    "enabling cache clear might impact latency, avoid excessive calls in tight loop"
                )
    ...
    def snapshot(
        ...
        logger.info(
            "memory usage @ '%s' : GPU %s, RAM %s",
            snapshot_marker,
            marker.gpu_memory_usage,
            marker.cpu_memory_usage,
        )
        if append:
    ...
    def report(self):
        ...
        logger.info(
            ...
            generate_event_report(
                [m.to_dict() for m in self._markers], max_memory_threshold=0.9
            ),
        )
...
def event_marker(
    event: str, device: Union[Device, int] = None, flush_ram: bool = False
):
    """
...
def generate_event_report(
    event_list: List[Dict[str, Union[int, str]]], max_memory_threshold: float
) -> str:
    """
    ...
    gpu_usage = (
        lambda event: event["gpu_memory_usage"] + event["delta_gpu_memory_usage"]
    )
    cpu_usage = (
        lambda event: event["cpu_memory_usage"] + event["delta_cpu_memory_usage"]
    )
    ...
    for event in event_list:
        ...
        stream.write(
            "\n{:<65} {:>20}{:>5} | {:>12} {:>12}{}|  {:>12} {:>12}{}|".format(
                ...
                "{:.0%}".format(duration / tot_time),
                convert_bytes(event["delta_gpu_memory_usage"]),
                convert_bytes(gpu_mem),
                gpu_marker,
                convert_bytes(event["delta_cpu_memory_usage"]),
                convert_bytes(cpu_mem),
                cpu_marker,
            )
        )
