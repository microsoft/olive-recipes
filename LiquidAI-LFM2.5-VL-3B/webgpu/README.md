# LiquidAI-LFM2.5-VL-3B — WebGPU optimization
...
```
olive run --config LiquidAI-LFM2.5-VL-3B_webgpu_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-VL-3B_webgpu_fp16_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-VL-3B_webgpu_vision_int8.json
```
...
| --- | --- | --- | --- |
| `_webgpu_int4.json` | 2.01 GiB | 0.030 | 90.5% |
| `_webgpu_fp16_int4.json` | 2.03 GiB | 0.061 | 87.7% |
| llama.cpp Q4_K_M | 1.56 GiB | 0.024-0.025 | 92.0-92.3% |
| llama.cpp Q8_0 | 2.68 GiB | 0.0002-0.0005 | 98.8-99.2% |

INT4 trails Q4_K_M: 19% more KLD than llama.cpp's CPU backend (0.030 against 0.025), at 1.3x its
...
`_webgpu_fp16_int4.json` is the least accurate recipe here despite being the larger one (KLD 0.061
against 0.030): plain RTN costs more than the FP16 LM head saves.
