...
def llm_build_preparer_converter_args(num_hidden_layers, model_input_names, separate_position_ids=True, use_qairt_mpp=False, attention_indices=None, num_deepstack_layers=None):
    '''
    ...
    if not use_qairt_mpp:
        ...
        for input_param in converter_args_param:
            for input_name in model_input_names:
                ...
                elif input_name == 'deepstack_visual_embeds':
                    raise NotImplementedError(
                        "The Aimet-pro MPP converter args workflow does not support new feature including 'deepstack_visual_embeds'. Please use the QAIRT MPP converter args workflow by setting use_qairt_mpp=True."
                    )
                else:
        ...
        for input_name in model_input_names:
            if input_name in {'position_ids', 'swa_position_ids'} and separate_position_ids:
                ...
                if num_deepstack_layers is None:
                    raise ValueError("num_deepstack_layers must be provided when 'deepstack_visual_embeds' is included in model_input_names.")
                for i in range(num_deepstack_layers):
                    input_tensor_config=InputTensorConfig(name=f'deepstack_visual_embeds[{i}]',
                                                        source_model_input_layout='NONTRIVIAL', source_model_input_datatype='float32')
                    input_tensors.append(input_tensor_config)
            else:
                input_tensor_config=InputTensorConfig(name=input_name,
