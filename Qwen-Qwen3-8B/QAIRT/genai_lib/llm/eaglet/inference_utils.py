...
def llm_create_draft_tree(
    ...
    for i in range(depth):
        ...
        # check if out_hidden is a tuple (hidden states from all layers)
        # or just last layer hidden states tensor
        if isinstance(out_hidden, (tuple, list)):
            # out_hidden: tuple/list of tensors for each layer with [batch_size, seq_len, hidden_dim]
            # pick last hidden states followed by picking positions given by out_ids to get hidden state of parents
            input_hidden = out_hidden[-1][:, out_ids]
        else:
            # out_hidden: [batch_size, seq_len, hidden_dim]
            # pick all batches, positions given by out_ids, all hidden dimensions to get hidden state of parents
