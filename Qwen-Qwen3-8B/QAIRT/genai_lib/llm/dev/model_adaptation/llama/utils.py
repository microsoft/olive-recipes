...
def llm_create_causal_mask(prepared_1d_attn_mask, input_tensor, max_input_tokens, model_context_len, config, mask_neg = -100, cache_index = None, pad_to_left = True):
    '''
...
def llm_update_causal_mask(prepared_1d_attn_mask, input_tensor, max_input_tokens, model_context_len, model_id_or_path, mask_neg = -100, cache_index=None, pad_to_left=True):
    ...
    if is_transformers_greater_or_equal("4.53.0"):
        warnings.warn(
            "`llm_update_causal_mask` will be deprecated."
            "Please use `llm_create_causal_mask` instead.",
            DeprecationWarning
        )
        config = _get_config(model_id_or_path)
        config._attn_implementation = "eager"
        prepared_attention_mask = llm_create_causal_mask(prepared_1d_attn_mask, input_tensor, max_input_tokens, model_context_len, config,mask_neg = mask_neg, cache_index=cache_index, pad_to_left=pad_to_left)
        return prepared_attention_mask
    else:
