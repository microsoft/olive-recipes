...
"""  This file provides utilities that the pipeline would need to work with the adaptations made to Gemma model. """
...
def llm_update_causal_mask(prepared_1d_attn_mask, input_tensor, max_input_tokens, model_context_len, model_id_or_path, mask_neg = -100, cache_index=None, pad_to_left=True):
    ...
    We  call `create_causal_mask` API which is not specific to any one model.
...
def llm_create_position_embeddings(config, dtype=torch.float32, position_ids=None):
    ...
    params:
    1. config: model configuration to create the Gemma3nTextRotaryEmbedding object, expect config for backward compatibility, in future transformers, we only expect to pass the config, the one we have in docker, takes in the req argument
...
def _get_rotary_embedding(device, config=None):
    from transformers.models.gemma3n.modeling_gemma3n import Gemma3nTextRotaryEmbedding
    rotary_emb = Gemma3nTextRotaryEmbedding(config).to(device)
    return rotary_emb
