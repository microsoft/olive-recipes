...
"""
Utils required for Qwen 3VL ViT Onboarding
"""
...
def vit_get_position_embeddings(model_id_or_path, hidden_states, grid_thw):
    """
    Precompute RoPE (Rotary Position Embedding) position embeddings for vision transformer.

    This function extracts and preprocesses rotary position embeddings from a visual model,
    specifically designed to work with Qwen3VL vision models. The implementation is extracted
    from the original `Qwen3VLVisionModel.forward` method for modular use.

    Args:
        model_id_or_path: Path or identifier for the Qwen3VL model.
        hidden_states (torch.Tensor): Input hidden states tensor with shape (seq_len, hidden_dim)
                                    used to determine sequence length and target device.
        grid_thw (torch.Tensor): Grid tensor containing temporal, height, and width dimensions
                               for position embedding computation.

    Returns:
        tuple: A tuple containing:
            - position_embeddings_cos (torch.Tensor): Cosine components of position embeddings
            - position_embeddings_sin (torch.Tensor): Sine components of position embeddings
            Both tensors are moved to the same device as hidden_states.

    Note:
        This function assumes the user provides a 1-layer visual model containing the necessary
        rotary position embedding components, rather than a full vision transformer model.
    """
    qwen3vl_onelayer_model = _get_model(model_id_or_path)
    rotary_pos_emb = qwen3vl_onelayer_model.visual.rot_pos_emb(grid_thw)
...
class Conv2dsInplaceConv3d(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        **Qwen 2.5VL/3VL - specific assumption (Vision Patch Embed setting):**
        ...
        Raises:
            ValueError: If the input (T, H, W) does not exactly match the effective 3D kernel size.
                        This adapter is specific to the Qwen 2.5VL/3VL Vision Patch Embed configuration.
        """
        ...
        input_thw = x.shape[-3:]
        error_msg = (
            f"This adaptation is specific to Qwen 2.5VL/3VL style Vision Patch Embed setting "
...
def _get_model(model_id_or_path):
    ...
    config.text_config.num_hidden_layers = config.vision_config.depth = 1
    config.vision_config.fixed_grid_thw = [1, 1, 1]  # Set a dummy fixed_grid_thw since the adapted ViT class `QcQwen3VLVisionModel` expects this attribute in initialization
