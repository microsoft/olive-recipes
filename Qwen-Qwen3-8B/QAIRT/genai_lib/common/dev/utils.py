...
def get_kv_length(past_key_values: Optional[Union[Cache, tuple]],
                  layer_index: Optional[int] = 0):
    if past_key_values is None:
        return 0
    elif isinstance(past_key_values, Cache):
        return past_key_values.get_seq_length(layer_index)
    elif isinstance(past_key_values, tuple):
        return past_key_values[layer_index][1].shape[-2]
