    ...
    def __init__(self, lora_layer: PeftLoraLayer):
        ...
        for i, scaling in enumerate(self.scaling):
            if isinstance(scaling, torch.Tensor) and scaling.size():
                ...
                if lora_layer.weight.ndim == 2:
                    self.scaling[i] = torch.nn.Parameter(scaling.view(1, scaling.size()[0]), requires_grad=False).to(scaling.device)
                elif lora_layer.weight.ndim == 4:
                    self.scaling[i] = torch.nn.Parameter(scaling.view(1, scaling.size()[0], 1, 1), requires_grad=False).to(scaling.device)
                else:
                    raise ValueError(f"Only 2D (Linear) and 4D (Conv2d) layers supported, encountered {lora_layer.weight.ndim}D layers")
