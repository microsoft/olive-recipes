# LiquidAI-LFM2.5-Audio-1.5B-JP — CPU optimization
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_cpu_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_cpu_int8.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_cpu_audio_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_cpu_audio_int8.json
```
...
ASR is character error rate on 50 random FLEURS ja_jp test clips (10.3 minutes). TTS is 10 Japanese
sentences spoken with the model card's sampling, transcribed by Whisper large-v3-turbo and scored
against the text. Punctuation and spacing are ignored. The GGUF rows are LiquidAI's release run
through its llama.cpp runner.

| recipe | size | ASR CER, Xeon / M3 Ultra | TTS CER, Xeon / M3 Ultra |
| --- | --- | --- | --- |
| int4 | 1.31 GB | 6.85% / 6.43% | 2.45% / 1.84% |
| int8 | 1.97 GB | 6.77% / 6.77% | 1.84% / 1.84% |
| GGUF Q4_0 | 1.07 GB | 6.89% | 2.45% |
| GGUF Q8_0 | 1.82 GB | 6.94% | 1.84% |
| GGUF F16 | 3.33 GB | 6.85% | 1.84% |
