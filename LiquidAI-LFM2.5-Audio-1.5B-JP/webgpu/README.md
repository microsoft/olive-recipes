# LiquidAI-LFM2.5-Audio-1.5B-JP — WebGPU optimization
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_webgpu_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_webgpu_audio_int4.json
```
...
```
olive run --config LiquidAI-LFM2.5-Audio-1.5B-JP_webgpu_audio_int8.json
```
...
Measured on an Apple M3 Ultra (Dawn over Metal). ASR is character error rate on 50 random FLEURS
ja_jp test clips (10.3 minutes). TTS is 10 Japanese sentences spoken with the model card's sampling,
transcribed by Whisper large-v3-turbo and scored against the text. Punctuation and spacing are
ignored.

| recipe | size | ASR CER | TTS CER |
| --- | --- | --- | --- |
| int4 | 1.22 GB | 7.02% | 2.45% |
| int8 | 1.88 GB | 7.11% | 1.84% |
