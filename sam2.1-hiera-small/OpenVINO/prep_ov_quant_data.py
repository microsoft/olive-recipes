...
def _safe_extract(zf, member, target_dir):
    ...
    if not target.startswith(os.path.realpath(str(target_dir)) + os.sep) and target != os.path.realpath(str(target_dir)):
...
def generate_decoder_inputs():
    ...
    for idx in pbar:
        ...
        np.savez(save_path, image_embeddings=image_embeddings.numpy(),
