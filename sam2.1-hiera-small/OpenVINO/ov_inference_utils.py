...
class FacebookSam2_1HieraSmall(
    ...
    def __init__(
        ...
        if device is not None and execution_provider is not None:
            ...
            print(f"Successfully created ONNX Runtime sessions with specified EP '{execution_provider}' and device '{device_str}'.")
        else:
    ...
    def segment_image(
        ...
        pixels, (scale_x, scale_y) = resize_normalize_adjust_dim(
            image=image,
        ...
        point_coords, point_labels = self._encode_prompt(
            prompt=prompt,
            scale_x=scale_x,
            scale_y=scale_y
        )
        ...
        resized_masks = np.empty(
            (output_masks.shape[0], image.height, image.width),
        ...
        return IPromptedImageSegmentationModel.Prediction(
            mask=binary_mask
        )
