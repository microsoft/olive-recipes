...
def md_inputs(batch_size, torch_dtype):
    return {input_name: torch.rand((batch_size, *input_shape), dtype=torch_dtype) for input_name, input_shape in zip(config.md_input_names, config.ms_input_shapes)}
...
def ve_generate_quant_data(num_samples):
    ...
    processor = Sam2Processor.from_pretrained(config.model_name)
