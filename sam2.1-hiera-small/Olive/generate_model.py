...
def window_partition(x, window_size):
    ...
    x = x.view(Hp // window_size, window_size, Wp // window_size, window_size, C)
...
def window_unpartition(windows, window_size, pad_hw, hw):
    ...
    x = windows.reshape(
        Hp // window_size, Wp // window_size, window_size, window_size, -1
    )
    x = x.permute(0, 2, 1, 3, 4).reshape(B, Hp, Wp, -1)
...
class ModMultiScaleBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # MLP
        x = x + self.model.drop_path(self.model.mlp(self.model.norm2(x)))
        return x
...
def main():
    ...
    de_inputs = {"image_embeddings": torch.rand([1, 256, 64, 64]), "high_res_features1":  torch.rand([1, 32, 256, 256]),
             "high_res_features2": torch.rand((1, 64, 128, 128)), "unnorm_coords": torch.randn((1,5,2)), "labels": torch.ones((1, 5))}
    ...
    if check:
        onnx.save(simplified_encoder_onnx_model, "sam21_vision_encoder_sim.onnx")
        print("Simplified encoder model saved to: sam21_vision_encoder_sim.onnx")
    else:
        print("Simplification of encoder model failed. Please check the model.")

    decoder_onnx_model = onnx.load("sam21_mask_decoder.onnx")
    simplified_decoder_onnx_model, check = simplify(decoder_onnx_model)
    
    if check:
        onnx.save(simplified_decoder_onnx_model, "sam21_mask_decoder_sim.onnx")
        print("Simplified decoder model saved to: sam21_mask_decoder_sim.onnx")
    else:
        print("Simplification of decoder model failed. Please check the model.")
